...
- `git clone git@github.com:%ИМЯ_АККАУНТА%/first-project.git` - Клонировать удалённый репозиторий на локальный компьютер. После этого `git push` можно делать без флагов
- `git branch` - Просмотреть локальны ветки проекта. т.е. На локальном компьютере
- `git branch <название_ветки>` - Создать ветку. Название ветки в Git может состоять из букв, цифр, а также включать любой из четырёх символов: `.`, `-`, `_`, `/`
- `git branch -a` - просмотреть все ветки, в том числе все доступные на GitHub
- `git checkout <название_ветки>` - Переключиться на другую ветку
- `git checkout -b <название_ветки>` - Создать ветку и сразу переключиться на неё
- `git diff <название_ветки1> <название_ветки2>` - Сравнить ветки 1 и 2. Нумерация веток здесь это их иерархический порядок
- `~` - суффикс навигации для `git diff`. Используется как ~N, где N — это число. Он отсчитывает от заданного коммита N коммитов назад во времени. Нумерация начинается с нуля: commit~0 — это сам коммит, commit~1 — предыдущий, commit~2 — предшествующий предыдущему и так далее.
- `git merge <название_присоединяемой_ветки>` - выполнить присоединение ветки
- `git branch -D <название_ветки_донора>` - Удалить принудительно ветку после объединения
- `git branch --d <название_ветки_донора>` - безопасно удалить ветку только если она была полностью объединена с другой 
